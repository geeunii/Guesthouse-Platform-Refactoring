...
**[해결 과정]**
- **소켓 및 런타임 종료:** `systemctl stop docker.socket` 후 `lsof`로 프로세스 정리.
- **포트 전략 수정:** Admin Server의 외부 포트를 `8081`로 변경하여 배포.

```bash
# 기존 8080 충돌 방지를 위해 Host Port를 8081로 매핑하여 실행
docker run -d \
  -p 8081:8080 \
  --name admin-server \
  --network vpc-network \
...
* 포트 충돌 해결 및 User/Admin 컨테이너 공존 환경 구축.
...
* 서버 분리 후, 관리자 페이지 접속 시 502 에러 발생.
* Nginx가 Private Subnet에 있는 Admin Server를 찾지 못함.

**[해결 과정]**

* **Upstream 설정 구체화:** Nginx 설정에 `/api/admin/` 블록을 추가하고, `localhost`가 아닌 **실제 Private IP**를 명시.

```nginx
# /etc/nginx/sites-available/default
location /api/admin/ {
    # localhost(X) -> Private IP(O) 명시적 지정
...
* **방어적 파싱 로직(`parseSafe`) 구현:** 응답을 `Object`로 받아 타입에 따라 유연하게 처리.

```java
// AI가 String으로 주든 List로 주든 무조건 List<String>으로 변환
private List<String> parseSafe(Object response) {
    if (response == null) return new ArrayList<>();
    
    // 단순 문자열로 온 경우 콤마(,)로 잘라서 리스트화
